...
class ABFinder_Database
{
	...
	public function query_local_vehicles($query)
	{
		...
		switch ($query['key']) {
			...
			case 'makeSelect':
				if (!isset($query['year'])) {
					return array('id'=> 'modelSelect', 'items'=> [], 'select'=> 'make', 'key'=> 'makeSelect', 'defaultText'=> 'Make');
				}
			...
			case 'modelSelect':
				if(!isset($query['year']) || !isset($query['make'])) {
					return array('id'=> 'submodelSelect', 'items'=> [], 'select'=> 'model', 'key'=> 'modelSelect', 'defaultText'=> 'Model');
				}
			...
			case 'submodelSelect':
				if(!isset($query['year']) || !isset($query['make']) || !isset($query['model'])) {
					return array('id'=> 'bodytypeSelect', 'items'=> [], 'select'=> 'submodel', 'key'=> 'submodelSelect', 'defaultText'=> 'Submodel');
				}
			...
			case 'bodytypeSelect':
				if(!isset($query['year']) || !isset($query['make']) || !isset($query['model']) || !isset($query['submodel'])) {
					return array('id'=> 'qualifierSelect', 'items'=> [], 'select'=> 'bodytype', 'key'=> 'bodytypeSelect', 'defaultText'=> 'Bodytype');
				}
			...
			case 'qualifierSelect':
				if(!isset($query['year']) || !isset($query['make']) || !isset($query['model']) || !isset($query['submodel']) || !isset($query['bodytype'])) {
					return array('id'=> 'qualifierSelect', 'items'=> [], 'select'=> 'qualifier', 'key'=> 'qualifierSelect', 'defaultText' => 'Qualifier');
				}
				$sql = "SELECT DISTINCT qualifier FROM {$this->table_name} WHERE year LIKE '%{$query['year']}%' AND make LIKE '%{$query['make']}%' AND model LIKE '%{$query['model']}%' AND submodel LIKE '%{$query['submodel']}%' AND bodytype LIKE '%{$query['bodytype']}%' AND qualifier != '' AND status = 0 ORDER BY qualifier ASC";
				$result = $this->wpdb->get_results($sql, ARRAY_A);
				$store_query_result['value'] = $result;
				if (!empty($result)) {
	...
	{
		global $wpdb;
		$wpdb->query("TRUNCATE TABLE {$wpdb->prefix}abfinder_vehicle_query_history");
	}
